...
#'
#' Augments the data_set of a TMB analysis object.
#'
#' @param x The tmb_analysis object to augment.
#' @param terms The terms to augment the data_set with.
...
augment.tmb_analysis <- function(x, terms = c("fit", "residual"), conf_level = 0.95, ...) {
  ...
  for (term in terms) {
    ...
    reported <- reported[!colnames(reported) %in% colnames(data_set)]
    data_set %<>% dplyr::bind_cols(reported)
  }
  data_set
}
